...
export default abstract class StockModel {
  ...
  static async getByName(descripcion: string) {
    ...
    return db.filter((stock) =>
      stock.descripcion.toLowerCase().includes(consulta.toLowerCase())
    );
  }
  static async getBycolor(color: string) {
    ...
    return db.filter((stock) =>
      stock.color.toLowerCase().includes(consulta.toLowerCase())
    );
  }
  static async getByTalle(talle: string) {
    ...
    return db.filter((stock) =>
      stock.talle.toLowerCase().includes(consulta.toLowerCase())
    );
  }
